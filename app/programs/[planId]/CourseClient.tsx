...
// Reuse the type inference or define it explicitly
...
// --- COMPONENTS ---
// (Components like HeroSection, PainSection need to be here or imported. 
// Since they were in the original file, I will include them here for completeness.
// For brevity in this artifacts, I will include the full content if I can, 
// OR I will assume the user prefers the file to be split. 
// Given the constraints, I will copy the FULL components from the original file 
// but condensed if possible? No, I must provide working code.)
//
// Actually, to ensure the code works, I should probably copy the definition of HeroSection etc.
// from the original file I read in Step 41.

const HeroSection = ({ course }: { course: CourseDataType }) => {
    return (
...
const PainSection = ({ course }: { course: CourseDataType }) => {
    return (
...
const SolutionSection = ({ course }: { course: CourseDataType }) => {
    return (
...
const CurriculumAccordion = ({ course }: { course: CourseDataType }) => {
                    ...
                    {course.curriculum.map((module, idx) => (
                        <div key={idx} className="border border-gray-200 rounded-2xl overflow-hidden bg-white shadow-sm">
                            ...
                            >
                                ...
                                <svg
                                    className={`w-6 h-6 text-gray-400 transition-transform flex-shrink-0 ml-4 ${openIndex === idx ? 'rotate-180' : ''
                                        }`}
                                    fill="none"
                                    viewBox="0 0 24 24"
                                    stroke="currentColor"
...
const PremiumSection = ({ course }: { course: CourseDataType }) => {
    return (
                ...
                <div className="grid md:grid-cols-2 gap-6 mb-12">
                    {pricing.options.map((option) => (
                        <div
                            ...
                            className={`cursor-pointer p-8 rounded-2xl border-2 transition-all relative ${selectedOptionId === option.id
                        ...
                        >
                            ...
                            <div className="flex items-center gap-3 mb-4">
                                <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${selectedOptionId === option.id ? 'border-[#00B050]' : 'border-gray-300'
                                    }`}>
                ...
                <div className="bg-gradient-to-br from-[#00B050] to-[#009645] text-white rounded-2xl p-8 shadow-xl">
                    <div className="max-w-2xl mx-auto">
                        ...
                        <button
                            ...
                            className={`w-full py-5 rounded-xl font-bold text-xl transition-all ${isProcessing || !isSdkReady
...
export default function CourseClient({ course }: CourseClientProps) {
    ...
    return (
        <div className="min-h-screen bg-white font-sans antialiased">
            ...
            <PainSection course={course} />
            <SolutionSection course={course} />
            <CurriculumAccordion course={course} />
            <PremiumSection course={course} />
            <PricingSection course={course} isSdkReady={isSdkReady} />
            <FooterCTA />
        </div>
    );
}
